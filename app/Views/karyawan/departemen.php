        ...
        <div class="row">
            <div class="col-md-2 p-0 sidebar">
                ...
                <ul class="nav flex-column mt-2">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="<?= base_url('departemen') ?>"><i class="fas fa-building"></i> Departemen</a></li>
                        ...
                        <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center flex-wrap">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                <div class="input-group input-group-sm w-auto">
                                    ...
                                    <input type="search" class="form-control" placeholder="Cari..." data-table-search="#tableDepartemen">
                                </div>
                                <a href="<?= base_url('departemen/create') ?>" class="btn btn-primary btn-sm"><i class="fas fa-plus me-1"></i>Tambah Departemen</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-hover" id="tableDepartemen">
                                    <thead><tr><th>#</th><th>Nama Departemen</th><th>Jumlah Karyawan</th><th>Aksi</th></tr></thead>
                                    <tbody>
                                        <?php foreach($departemen as $key => $row): ?>
                                        <tr>
                                            <td><?= $key + 1 ?></td>
                                            <td><?= $row['nama_departemen'] ?></td>
                                            <td><?= $row['jumlah_karyawan'] ?? 0 ?> orang</td>
                                            <td>
                                                <a href="<?= base_url('departemen/edit/' . $row['id']) ?>" class="btn btn-sm btn-warning btn-action"><i class="fas fa-edit"></i></a>
                                                <a href="<?= base_url('departemen/delete/' . $row['id']) ?>" class="btn btn-sm btn-danger btn-action" onclick="return confirm('Yakin?')"><i class="fas fa-trash"></i></a>
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>
                                    </tbody>
